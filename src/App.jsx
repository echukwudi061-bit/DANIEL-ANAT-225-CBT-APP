...
export default function App() {
  ...
  // 1. Questions (Initialized with SHUFFLE)
  const [questions, setQuestions] = useState(() => {
    const saved = localStorage.getItem('cbt_questions');
    // If no saved questions, use the shuffled INITIAL_QUESTIONS
  ...
  const handleCSVUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      if (newQuestions.length > 0) {
        // Shuffle new imports as well? Optional, but here we just append.
  ...
  const handleResetDefaults = () => {
    if(window.confirm("Reset all settings and questions to default? This clears your changes.")) {
        localStorage.clear();
        window.location.reload(); // This will trigger re-initialization with Shuffled Default Questions
    }
  };
      ...
      <div className="min-h-screen bg-gray-50 p-4 font-sans text-gray-900 overflow-y-auto">
        <div className="max-w-3xl mx-auto space-y-6">
          <div className="bg-white p-6 md:p-8 rounded-2xl shadow-xl text-center">
            ...
            <h1 className="text-xl md:text-2xl font-bold text-blue-600 mb-6 uppercase tracking-tight border-b pb-4">
              {testTitle}
            </h1>
